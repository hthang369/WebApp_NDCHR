...
namespace ExpertERP.BusinessEntities
{
	...
	//Class:PPCostingNormAlternativesController
	//Created Date:Tuesday, September 20, 2016
	//-----------------------------------------------------------
	
	public class PPCostingNormAlternativesController:BaseBusinessController
	{
		public PPCostingNormAlternativesController()
		{
			dal= new DALBaseProvider("PPCostingNormAlternatives",typeof(PPCostingNormAlternativesInfo));
		}
        ...
        public DataSet GetAllDataByArrayPPCostingNormProductID(string[] arrPPCostingNormProductID)
        {
            string strQuery = string.Format(@"
                                            SELECT  *
                                            FROM    dbo.PPCostingNormAlternatives
        ...
        {
            string strSubQuery = DALUtil.GenerateColumnsFromTable("PPNormAlternatives", dal.TableName);
            string strQuery = string.Format(@"{0}
                            ...
                            EXEC sp_executesql @sql
                                            ", strSubQuery, "PPNormAlternatives", string.Join(",", arrPPNormID)
                                            );
            return GetDataSet(strQuery);
        }
        ...
        {
            string strQuery = string.Format(@"
                                            UPDATE   dbo.PPCostingNormAlternatives
                                            SET AAStatus='{1}'
                                            WHERE   FK_PPCostingNormRMID IN (
                                                    SELECT  PPCostingNormRMID
                                                    FROM    dbo.PPCostingNormRMs
